...
const ReportPopUp = () => {
    ...
    return (
        isActive ? (
            <Slide direction='right' className='absolute top-2 right-2' duration={400}>
                <div className='text-white flex flex-col bg-[#1E1E1E] pb-4 p-3 rounded-lg cursor-pointer'>
        ...
        ) : (<TbUrgent onClick={() => { setIsActive(true) }} size={55} className='fixed p-2 text-white transition duration-300 bg-red-500 border-4 border-red-500 rounded-lg cursor-pointer top-2 right-2 hover:bg-white hover:text-red-500' />)
    )
}
